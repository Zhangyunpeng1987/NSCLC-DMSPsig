...
#Functional analysis of GE
#Perform Jekyll correlation analysis on 10 GE using the H gene set from Msigdb database
...
#Read in the H gene set
library(msigdbr)
msigdbr_species() #List some species

#Select gene set
human_KEGG = msigdbr(species = "Homo sapiens",category = "H" ) %>% dplyr::select(gs_name,gene_symbol)#choose between gene symbol or ID here
...
library(ggplot2)
library(clusterProfiler)
data(gcSample)# Call gcsample data
...
xx.kegg <- compareCluster(
  ...
  fun = "enrichKEGG",
  organism = "hsa"  # Designate the species as humans
)
...
p1 <- DimPlot(cancer.epi, reduction = "umap", 
              ...
              raster = FALSE, 
              group.by = "GEtype",
              cols = colors_13) + SeuratAxes() #+ NoLegend()
p1
...
# 2. Set color (consistent with mutation order)
...
p2 <- DimPlot(cancer.epi, reduction = "umap", 
              ...
              raster = FALSE, 
              group.by = "mutation",
              cols = mycolors) + SeuratAxes() #+ NoLegend()
...
library(ggpubr)     # Significant annotation
library(ggplot2)
# 1. Clean data (ensure there is no NA)
df <- na.omit(corsce@meta.data[, c("GE1_UCell", "cellmodule")])

# 2. Set color (consistent with the order of cell modules)
mycolors <- c("#70cdbe", "#80C1D7", "#968ab7", "#d586b3", "#eb7e60")
names(mycolors) <- unique(df$cellmodule)  # Ensure name matching

# 3. Calculate the overall P-value of Kruskal Wallis
...
#write.csv(gene1,file = "./gene_cnv.csv")
...
gene2 <- rbind(gene2,cnv)
#gene_up <- subset(gene,name %in% gene_name1)
#gene_down <- subset(gene,name %in% gene_name2)
library(circlize)
circos.clear() # Drawing initialization
...
# Initialize the list that stores all grouping results
all_ge_data <- list()
...
for (ge_num in 1:13) {
  ...
  # 1. Extract cells from the current GE group
  GE_subset <- subset(cancerepi, GEtype == ge_group)
  
  # 2. Extract expression matrix and filter cells
  expr <- infercnv_obj@expr.data
  expr.cancer <- expr[, colnames(expr) %in% GE_subset$cellid, drop = FALSE]
  
  # 3. Centralize expression data and calculate the average CNV score of genes
  expr_centered <- expr.cancer - 1
  gene_means <- rowMeans(expr_centered)
  
  # 4. Integrate gene location information and create a Gene column
  gene_info <- infercnv_obj@gene_order
  cnv_data <- data.frame(
    Gene = rownames(gene_info),  # Explicitly creating a Gene column
    gene_info,
    cnv_score = gene_means[match(rownames(gene_info), names(gene_means))]
  )
  rownames(cnv_data) <- NULL  # Clear row names to avoid duplication
  
  # 5. Screening for extreme genes of CNV
  top_gain <- cnv_data %>% 
  ...
  # 6. Merge the current grouping results
  ge_result <- rbind(top_gain, top_loss) %>% 
    mutate(GE_group = ge_group)  # Add group identifier
  
  # 7. Store in the overall list
  all_ge_data[[ge_group]] <- ge_result
}

# 8. Merge all grouped data
final_combined_data <- do.call(rbind, all_ge_data) %>%
  # Reorder the order to place Gene first
  select(Gene, everything())

# 9. Save the final result
write.csv(final_combined_data, 
          file = "GE1-GE13_combined_cnv_results.csv",
          row.names = FALSE)  # No more need for row names

# 10. View data structure
...
#Search for the top ten differentially expressed genes in each sub cluster of each cell module
#save(subsce3,file = "./subsce_module.RData")
...
#exprset <- data.frame(t(exprset))
clinical <- train.meta
#Read in the marker genes of each cell module
...
library(scales)
#clinical_LUAD1$module1_score <- AddModuleScore(exprSet,features = module1.marker)
...
#Method 2: Calculate the scoring threshold for each cell module for each patient. Scores above the threshold are considered high, while scores below the threshold are considered low
#module1
...
best_threshold_surv <- surv_cutpoint(clinical,
                                     ...
                                     #minprop = 0.3,     # Minimum ratio to prevent finding thresholds that are too extreme
...
#palette = c("#9370db","#66bc90"))#c9beff
#xlim = c(0,28))#define color
